...
//   EMAIL: huynhquoctoan26@gmail.com
//-----------------------------------------------------------------------------------------------------------
...
//      |-- SERVO               | Header and lib source files for communicate with Servo via RS-485 Protocol
//      |-- MODBUS_TCP          | Header and lib source files for ModbusTCP Protocol
//   |-- src                    |
//      |-- main.cpp            | <--[THIS FILE]
...
struct Struct_Fixed_Modes
{
    ...
    {
        // HOME THETA
        const int Theta1 = 0, Theta2 = 155, Theta3 = -65, Theta4 = 0, Theta5 = -60, Theta6 = 0;
    } Home;
    struct Wakeup_Mode
    {
        // WAKE UP THETA
        const int Theta1 = 0, Theta2 = 90, Theta3 = 0, Theta4 = 0, Theta5 = -90, Theta6 = 0;
    } Wakeup;
    struct Sleep_Mode
    {
        // SLEEP THETA
        const int Theta1 = 0, Theta2 = 123, Theta3 = -33, Theta4 = 0, Theta5 = -90, Theta6 = 0;
    } Sleep;
} Fixed_Modes;
...
float theta1_old = 0, theta2_old = 0, theta3_old = 0, theta4_old = 0, theta5_old = 0, theta6_old = 0;
...
// FOR MOVE SERVO
long POS_SERVO_1 = 0, POS_SERVO_2 = 0, POS_SERVO_3 = 0, POS_SERVO_4 = 0, POS_SERVO_5 = 0, POS_SERVO_6 = 0;
long VEL_SERVO_1 = 0, VEL_SERVO_2 = 0, VEL_SERVO_3 = 0, VEL_SERVO_4 = 0, VEL_SERVO_5 = 0, VEL_SERVO_6 = 0;
...
unsigned int Speed_Find_Home = 8000;      // Tốc độ tìm home (để quay về gần cảm biến và tiến hành dò điểm giữa cảm biến home)
unsigned int Speed_Return_Origin = 12000; // Tốc độ quay về gốc khi xác định đã bị quay ngược hướng lúc tìm home
unsigned int Speed_Find_Origin = 4000;    // Tốc độ đi lấy vị trí ở 2 hướng tại cảm biến home
unsigned int Speed_Move_Origin = 2000;    // Tốc độ quay về điểm chính giữa của cảm biến khi biết vị trí 2 chiều quay tại cảm biến
// FOR ROBOT HARDWARE
...
float theta1_Init = 0, theta2_Init = 0, theta3_Init = 0, theta4_Init = 0, theta5_Init = 0, theta6_Init = 0;
float slider1_target = 0, slider2_target = 0, slider3_target = 0, slider4_target = 0, slider5_target = 0, slider6_target = 0;
bool EN_READY = false;
bool DONE_ALM = false;
bool FIND_NEAREST_SOLUTION = false;
float Theta1_IK_READY = 0, Theta2_IK_READY = 0, Theta3_IK_READY = 0, Theta4_IK_READY = 0, Theta5_IK_READY = 0, Theta6_IK_READY = 0;
...
bool EE4_Option = false;

bool ETH_Connect = false, L_State = false, D_State = false;
unsigned long interval = 500, previous_Time = 0, previous_Time_D = 0, current_Time = 0;
...
String convertBytesToHexString(byte bytes[])
{
    ...
    for (int i = 0; i < 6; i++)
    {
        // Chuyển đổi byte thành giá trị hex và thêm vào chuỗi
        char hex[3];
        sprintf(hex, "%02X", bytes[i]);
        hexString += hex;

        // Nếu không phải là phần tử cuối cùng, thêm dấu ":"
        if (i < 5)
        {
            hexString += ":";
        }
...
void Read_HMI()
{
    while (HMI.available())
    {
        ...
        if (HMI_BUFFER_DATA.indexOf(0x1A) != NULL_POS || HMI_BUFFER_DATA.indexOf(0x1B) != NULL_POS || HMI_BUFFER_DATA.indexOf(0x24) != NULL_POS)
            return;
        else
            ENABLE_BUZZER(10);
        // for (int i=0; i <= HMI_BUFFER_DATA.length(); i++)
...
{
    HMI.print(page + "." + object + "." + para + "=" + String('"') + content + String('"'));
    Write_End_Byte_HMI();
}
...
float GET_THETA_1()
{
    ...
    float theta_Servo = (float)(POS_SERVO_1 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_1_Ratio);
    return theta_Servo;
}
float GET_THETA_2()
{
    SERVO_GetActualPos(ID_2, &POS_SERVO_2);
    float theta_Servo = (float)(POS_SERVO_2 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_2_Ratio);
    return theta_Servo;
}
float GET_THETA_3()
{
    SERVO_GetActualPos(ID_3, &POS_SERVO_3);
    float theta_Servo = (float)(POS_SERVO_3 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_3_Ratio);
    return theta_Servo;
}
float GET_THETA_4()
{
    SERVO_GetActualPos(ID_4, &POS_SERVO_4);
    float theta_Servo = (float)(POS_SERVO_4 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_4_Ratio);
    return theta_Servo;
}
float GET_THETA_5()
{
    SERVO_GetActualPos(ID_5, &POS_SERVO_5);
    float theta_Servo = (float)(POS_SERVO_5 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_5_Ratio);
    return theta_Servo;
}
float GET_THETA_6()
{
    SERVO_GetActualPos(ID_6, &POS_SERVO_6);
    float theta_Servo = (float)(POS_SERVO_6 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_6_Ratio);
    return theta_Servo;
}
float GET_SPEED_1()
{
    SERVO_GetActualVel(ID_1, &VEL_SERVO_1);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_1 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_1_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
float GET_SPEED_2()
{
    SERVO_GetActualVel(ID_2, &VEL_SERVO_2);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_2 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_2_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
float GET_SPEED_3()
{
    SERVO_GetActualVel(ID_3, &VEL_SERVO_3);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_3 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_3_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
float GET_SPEED_4()
{
    SERVO_GetActualVel(ID_4, &VEL_SERVO_4);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_4 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_4_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
float GET_SPEED_5()
{
    SERVO_GetActualVel(ID_5, &VEL_SERVO_5);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_5 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_5_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
float GET_SPEED_6()
{
    SERVO_GetActualVel(ID_6, &VEL_SERVO_6);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_6 * 360) / (Pulse_Per_Revolution_PPR * Gearbox_6_Ratio); // dps = (pps*360)/PPR_Total
    return angular_Velocity;
}
unsigned int SPEED_SERVO_1(float degree_target, float degree_target_old, float time_target)
{
    ...
    long pulse_Difference = abs(DEG_TO_POS_1_CONV(degree_target) - DEG_TO_POS_1_CONV(degree_target_old));
...
{
    Serial.printf("Function 2 Received: degree_target = %0.2f, degree_target_old = %0.2f, time_target = %2d\n",
                  degree_target, degree_target_old, time_target);
...
{
    Serial.printf("Function 3 Received: degree_target = %0.2f, degree_target_old = %0.2f, time_target = %2d\n",
                  degree_target, degree_target_old, time_target);
...
{
    Serial.printf("Function 4 Received: degree_target = %0.2f, degree_target_old = %0.2f, time_target = %2d\n",
                  degree_target, degree_target_old, time_target);
...
unsigned int SPEED_SERVO_5(float degree_target, float degree_target_old, float time_target)
{
    Serial.printf("Function 5 Received: degree_target = %0.2f, degree_target_old = %0.2f, time_target = %2d\n",
                  degree_target, degree_target_old, time_target);
    long pulse_Difference = abs(DEG_TO_POS_5_CONV(degree_target) - DEG_TO_POS_5_CONV(degree_target_old));
    Serial.printf("Function 5 Received: pulse_Difference = %8ld, time = %2u\n", pulse_Difference, time_target);
    unsigned int speed = pulse_Difference / time_target;
    Serial.printf("Speed_5 = %ld\n\n", speed);
    return speed;
}
unsigned int SPEED_SERVO_6(float degree_target, float degree_target_old, float time_target)
{
    Serial.printf("Function 6 Received: degree_target = %0.2f, degree_target_old = %0.2f, time_target = %2d\n",
                  degree_target, degree_target_old, time_target);
    long pulse_Difference = abs(DEG_TO_POS_6_CONV(degree_target) - DEG_TO_POS_6_CONV(degree_target_old));
...
bool Check_T_0_3(float theta1, float theta2, float theta3, float Wx, float Wy, float Wz)
{
    // Serial.printf("Check the: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f\n", theta1, theta2, theta3);
    MatrixSize<4, 4> T_0_3_Matrix = T_0_1(L1, POS, d0, theta1) * T_1_2(L2, 0, 0, theta2) * T_2_3(L3, POS, d0, theta3) * T_3_4(0, NEG, d1, 0);
    printMatrix(T_0_3_Matrix);
    if ((T_0_3_Matrix.block<1, 1>(0, 3).value() - tolerance) <= Wx && (T_0_3_Matrix.block<1, 1>(0, 3).value() + tolerance) >= Wx)
    {
        if ((T_0_3_Matrix.block<1, 1>(1, 3).value() - tolerance) <= Wy && (T_0_3_Matrix.block<1, 1>(1, 3).value() + tolerance) >= Wy)
        {
            if ((T_0_3_Matrix.block<1, 1>(2, 3).value() - tolerance) <= Wz && (T_0_3_Matrix.block<1, 1>(2, 3).value() + tolerance) >= Wz)
            {
                // Serial.printf("W Position is OK: Wx_IK = %0.2f, Wy_IK = %0.2f, Wz_IK = %0.2f\n",
                //               T_0_3_Matrix.block<1, 1>(0, 3).value(), T_0_3_Matrix.block<1, 1>(1, 3).value(), T_0_3_Matrix.block<1, 1>(2, 3).value());
                return true;
            }
            else
                return false;
...
bool Check_T_0_6(const MatrixSize<3, 1> &EE_Pos_Matrix, const MatrixSize<3, 3> &R_0_6_Matrix, float theta1, float theta2, float theta3, float theta4, float theta5, float theta6)
{
    // Serial.printf("Check the: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f, theta4 = %0.2f, theta5 = %0.2f, theta6 = %0.2f\n",
    //               theta1, theta2, theta3, theta4, theta5, theta6);
    MatrixSize<4, 4> T_0_6_Matrix = Forward_Kinematics(theta1, theta2, theta3, theta4, theta5, theta6);
    ...
    float r11 = R_0_6_Matrix.block<1, 1>(0, 0).value(), r12 = R_0_6_Matrix.block<1, 1>(0, 1).value(), r13 = R_0_6_Matrix.block<1, 1>(0, 2).value();
    float r21 = R_0_6_Matrix.block<1, 1>(1, 0).value(), r22 = R_0_6_Matrix.block<1, 1>(1, 1).value(), r23 = R_0_6_Matrix.block<1, 1>(1, 2).value();
    float r31 = R_0_6_Matrix.block<1, 1>(2, 0).value(), r32 = R_0_6_Matrix.block<1, 1>(2, 1).value(), r33 = R_0_6_Matrix.block<1, 1>(2, 2).value();

    float T_r11 = T_0_6_Matrix.block<1, 1>(0, 0).value(), T_r12 = T_0_6_Matrix.block<1, 1>(0, 1).value(), T_r13 = T_0_6_Matrix.block<1, 1>(0, 2).value();
    float T_r21 = T_0_6_Matrix.block<1, 1>(1, 0).value(), T_r22 = T_0_6_Matrix.block<1, 1>(1, 1).value(), T_r23 = T_0_6_Matrix.block<1, 1>(1, 2).value();
    float T_r31 = T_0_6_Matrix.block<1, 1>(2, 0).value(), T_r32 = T_0_6_Matrix.block<1, 1>(2, 1).value(), T_r33 = T_0_6_Matrix.block<1, 1>(2, 2).value();

    if ((T_0_6_Matrix.block<1, 1>(0, 3).value() - tolerance) <= EEx && (T_0_6_Matrix.block<1, 1>(0, 3).value() + tolerance) >= EEx)
    {
        if ((T_0_6_Matrix.block<1, 1>(1, 3).value() - tolerance) <= EEy && (T_0_6_Matrix.block<1, 1>(1, 3).value() + tolerance) >= EEy)
        {
            if ((T_0_6_Matrix.block<1, 1>(2, 3).value() - tolerance) <= EEz && (T_0_6_Matrix.block<1, 1>(2, 3).value() + tolerance) >= EEz)
            {
                Serial.printf("EE Position is OK: EEx_IK = %0.2f, EEy_IK = %0.2f, EEz_IK = %0.2f\n",
                              T_0_6_Matrix.block<1, 1>(0, 3).value(), T_0_6_Matrix.block<1, 1>(1, 3).value(), T_0_6_Matrix.block<1, 1>(2, 3).value());
                if (r11 == T_r11 && r12 == T_r12 && r13 == T_r13 && r21 == T_r21 && r22 == T_r22 && r23 == T_r23 && r31 == T_r31 && r32 == T_r32 && r33 == T_r33)
                    return true;
                else
                    return false;
            }
...
String Inverse_Kinematics(const MatrixSize<3, 1> &EE_Pos_Matrix, const MatrixSize<3, 3> &R_0_6_Matrix)
{
    ...
    if (Out_Of_Workspace_TH1 == true || Out_Of_Workspace_TH2 == true || Out_Of_Workspace_TH3 == true)
    {
        if (Out_Of_Workspace_TH1 == true && Out_Of_Workspace_TH3 == true)
        {
            Serial.printf("OUT OF WORKSPACE! | EEx: %0.2f <= 0 | EEy: %0.2f >= 0 | EEz: %0.2f <= 0 |\n", EEx, EEy, EEz);
            return "OOW";
        }
    ...
    if (round(theta1_IK_T) >= -180 && round(theta1_IK_T) <= 180)
    {
        if (round(theta2_IK_1) >= -10 && round(theta2_IK_1) <= 165)
        {
            if (round(theta3_IK_1) >= -90 && round(theta3_IK_1) <= 90)
            {
                ...
                {
                    Serial.printf("FINAL - Solution 1 Found: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f\n\n",
            ...
            if (round(theta3_IK_2) >= -90 && round(theta3_IK_2) <= 90)
            {
                ...
                if (Check_T_0_3(theta1_IK, theta2_IK, theta3_IK, Wx, Wy, Wz) == true)
                {
                    Serial.printf("FINAL - Solution 2 Found: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f\n\n",
                                  theta1_IK, theta2_IK, theta3_IK);
                    Number_Of_Solutions++;
                }
                else
        ...
        if (round(theta2_IK_2) >= -10 && round(theta2_IK_2) <= 165)
        {
            if (round(theta3_IK_1) >= -90 && round(theta3_IK_1) <= 90)
            {
                ...
                if (Check_T_0_3(theta1_IK, theta2_IK, theta3_IK, Wx, Wy, Wz) == true)
                {
                    Serial.printf("FINAL - Solution 3 Found: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f\n\n",
                                  theta1_IK, theta2_IK, theta3_IK);
                    Number_Of_Solutions++;
                }
                else
            ...
            if (round(theta3_IK_2) >= -90 && round(theta3_IK_2) <= 90)
            {
                ...
                if (Check_T_0_3(theta1_IK, theta2_IK, theta3_IK, Wx, Wy, Wz) == true)
                {
                    Serial.printf("FINAL - Solution 4 Found: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f\n\n",
                                  theta1_IK, theta2_IK, theta3_IK);
                    Number_Of_Solutions++;
                }
                else
    ...
    {
        Serial.println("FIND_NEAREST_4_SOLUTION !");
        if (theta4_IK > 0)
    ...
    if (EE4_Option == true)
    {
        Serial.println("FIND_NEAREST_6_SOLUTION !");
        theta6_IK = theta6_IK;
        EE4_Option = false;
    }
    else
    ...
    float Sin_Theta5 = sqrt(1 - pow(Cos_Theta5, 2)); // Có 2 TH: Sin_Theta5 = +- Sqrt(...);

    theta5_IK_1 = atan2d(Sin_Theta5, Cos_Theta5);
    theta5_IK_2 = atan2d(-Sin_Theta5, Cos_Theta5);
    Serial.printf("Wrist Solution 1: theta4_IK = %0.2f, theta5_IK_1 = %0.2f, theta6_IK = %0.2f\n",
                  theta4_IK, theta5_IK_1, theta6_IK);
    Serial.printf("Wrist Solution 2: theta4_IK = %0.2f, theta5_IK_2 = %0.2f, theta6_IK = %0.2f\n",
                  theta4_IK, theta5_IK_2, theta6_IK);
    if (Check_T_0_6(EE_Pos_Matrix, R_0_6_Matrix, theta1_IK, theta2_IK, theta3_IK, theta4_IK, theta5_IK_1, theta6_IK) == true)
    {
        ...
        Serial.printf("IK DONE! - Result: theta1 = %0.2f, theta2 = %0.2f, theta3 = %0.2f, theta4 = %0.2f, theta5 = %0.2f, theta6 = %0.2f\n",
                      theta1_IK, theta2_IK, theta3_IK, theta4_IK, theta5_IK, theta6_IK);
        String result =
            "OK;" + String(theta1_IK) + ";" + String(theta2_IK) + ";" + String(theta3_IK) + ";" + String(theta4_IK) + ";" + String(theta5_IK) + ";" + String(theta6_IK) + ";";
        return result;
    }
    else
        Serial.println("Wrist Solution 1 is not match!");

    if (Check_T_0_6(EE_Pos_Matrix, R_0_6_Matrix, theta1_IK, theta2_IK, theta3_IK, theta4_IK, theta5_IK_2, theta6_IK) == true)
    {
...
String Inverse_Kinematics_No_Debug(const MatrixSize<3, 1> &EE_Pos_Matrix, const MatrixSize<3, 3> &R_0_6_Matrix)
{
    Number_Of_Solutions = 0;
    float EEx = EE_Pos_Matrix.block<1, 1>(0, 0).value();
    float EEy = EE_Pos_Matrix.block<1, 1>(1, 0).value();
    ...
    // Serial.println("Wrist Solution 1 is not match!");
    if (Check_T_0_6(EE_Pos_Matrix, R_0_6_Matrix, theta1_IK, theta2_IK, theta3_IK, theta4_IK, theta5_IK_2, theta6_IK) == false)
    {
        // Serial.println("Wrist Solution 2 is not match!");
    }
    else
...
void Check_Change_Page_Fcns()
{
    ...
    else if (HMI_BUFFER_DATA.indexOf("KI_SLI") != NULL_POS)
    {
        ...
        Write_HMI("st2", "val", (unsigned int)map(GET_THETA_2() + THETA2_OFFSET, 0, 135, 0, 135));
    ...
    else if (HMI_BUFFER_DATA.indexOf("KI_ENC") != NULL_POS)
    {
        ...
        Write_HMI("t5", "txt", roundFloatToString(GET_THETA_5(), 1));
        Write_HMI("t6", "txt", roundFloatToString(GET_THETA_6(), 1));
        FK_Jog = Forward_Kinematics(GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
        Write_HMI("xfk", "txt", roundFloatToString(FK_Jog.block<1, 1>(0, 3).value(), 1));
        Write_HMI("yfk", "txt", roundFloatToString(FK_Jog.block<1, 1>(1, 3).value(), 1));
        Write_HMI("zfk", "txt", roundFloatToString(FK_Jog.block<1, 1>(2, 3).value(), 1));
        clearBufferData();
    }
    ...
    else if (HMI_BUFFER_DATA.indexOf("EP") != NULL_POS)
    {
        CURRENT_PAGE = "EP";
        Write_HMI("ethst", "pco", 63488);
        Write_HMI("ethst", "txt", "Disconnected!");
        Write_HMI("ethbt", "txt", "Connect");
...
void Move_Robot_Fixed_Modes(const Fixed_Modes_Fcns &fixed_modes, unsigned int speed)
{
    Serial.println("Move_Robot_Fixed_Modes");
    ...
    Speed_2 = SPEED_SERVO_2(fixed_modes.Theta2 - THETA2_OFFSET, theta2_old - THETA2_OFFSET, speed);
    ...
    SERVO_MoveSingleAxisAbsPos(ID_2, DEG_TO_POS_2_CONV(fixed_modes.Theta2 - THETA2_OFFSET), Speed_2);
    delayMicroseconds(1500);
...
void MOVE_ORIGIN_SERVO_1()
{
    ...
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_1, (long)round(abs(CW_Pos - CCW_Pos) / 2), Speed_Move_Origin);
    while (GET_SPEED_1() != 0)
    {
        Write_HMI("j1h", "bco", (digitalRead(HOME_SENSOR_J1) ? 2016 : 8518));
    }
...
void MOVE_ORIGIN_SERVO_2()
{
    ...
    Serial.printf("Diff Pos: %ld\n", abs(CW_Pos - CCW_Pos));
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_2, (long)round(abs(CW_Pos - CCW_Pos) / 2), (unsigned long)Speed_Move_Origin * 2);
...
void MOVE_ORIGIN_SERVO_3()
{
    ...
    Serial.printf("Diff Pos: %ld\n", abs(CW_Pos - CCW_Pos));
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_3, (long)round(abs(CW_Pos - CCW_Pos) / 2), (unsigned long)Speed_Move_Origin);
    Serial.println("Move to origin");
    while (GET_SPEED_3() != 0)
    {
        Write_HMI("j3h", "bco", (digitalRead(HOME_SENSOR_J3) ? 2016 : 8518));
...
void MOVE_ORIGIN_SERVO_4()
{
    ...
    Serial.printf("Diff Pos: %ld\n", abs(CW_Pos - CCW_Pos));
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_4, (long)round(abs(CW_Pos - CCW_Pos) / 2), (unsigned long)Speed_Move_Origin * 2);
    Serial.println("Move to origin");
    while (GET_SPEED_4() != 0)
    {
        Write_HMI("j4h", "bco", (digitalRead(HOME_SENSOR_J4) ? 2016 : 8518));
...
void MOVE_ORIGIN_SERVO_5()
{
    ...
    Serial.printf("Diff Pos: %ld\n", abs(CW_Pos - CCW_Pos));
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_5, (long)round(abs(CW_Pos - CCW_Pos) / 2), (unsigned long)Speed_Move_Origin * 2);
    Serial.println("Move to origin");
    while (GET_SPEED_5() != 0)
    {
        Write_HMI("j5h", "bco", (digitalRead(HOME_SENSOR_J5) ? 2016 : 8518));
...
void MOVE_ORIGIN_SERVO_6()
{
    ...
    // Serial.printf("Diff Pos: %ld\n", abs(CW_Pos - CCW_Pos));
    delay(200);
    SERVO_MoveSingleAxisIncPos(ID_6, (long)round(abs(CW_Pos - CCW_Pos) / 2), Speed_Move_Origin);
    while (GET_SPEED_6() != 0)
    {
        Write_HMI("j6h", "bco", (digitalRead(HOME_SENSOR_J6) ? 2016 : 8518));
    }
...
void Joint_5_Calibration()
{
    ...
    else
    {
        ...
        if (Act_Pos <= 0)
        {
            // do
            // {
            //   SERVO_MoveSingleAxisAbsPos(ID_5, DEG_TO_POS_5_CONV(-90), Speed_Find_Home);
            // } while (digitalRead(HOME_SENSOR_J5) == false);
            // SERVO_MoveStop(ID_5);
            do
            {
...
void Update_Data_HMI()
{
    ...
    Write_HMI("x", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(0, 3).value() * 10) / 10, 1));
    Write_HMI("y", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(1, 3).value() * 10) / 10, 1));
    Write_HMI("z", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(2, 3).value() * 10) / 10, 1));
...
void Home_Page()
{
    ...
    else if (HMI_BUFFER_DATA.indexOf("OFF") != NULL_POS)
    {
        if (SYSTEM_STATUS == true)
        {
            ...
            DISABLE_ALL_SERVO();
            String LAST_THETA =
                "LAST_THETA;" + String(GET_THETA_1()) + ";" + String(GET_THETA_2()) + ";" + String(GET_THETA_3()) +
                ";" + String(GET_THETA_4()) + ";" + String(GET_THETA_5()) + ";" + String(GET_THETA_6()) + ";";
    ...
    if (SYSTEM_STATUS == true)
    {
        ...
        else if (HMI_BUFFER_DATA.indexOf("SLP") != NULL_POS)
        {
    ...
    // {
    //   if (WAKEUP_No == 0 && theta1_old == 0 && digitalRead(HOME_SENSOR_J1) == true)
    //   {
...
void Update_Slider_State()
{
    ...
    {
        slider1_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 180, -90, 90);
    ...
    {
        slider2_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 135, 0, 135);
    ...
    {
        slider3_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 90, -45, 45);
    ...
    {
        slider4_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 180, -90, 90);
    ...
    {
        slider5_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 250, -125, 125);
    ...
    {
        slider6_target = (float)map(splitString(HMI_BUFFER_DATA, ";", 1).toFloat(), 0, 360, -180, 180);
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS1") != NULL_POS)
    {
        ...
        if (slider1_target_temp >= -90 && slider1_target_temp <= 90)
        {
            ...
            SERVO_MoveSingleAxisAbsPos(ID_1, DEG_TO_POS_1_CONV(roundToNearestHalfDeg(slider1_target)), Move_Speed);
        }
        else
        {
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS2") != NULL_POS)
    {
        ...
        if (slider2_target_temp >= 0 && slider2_target_temp <= 135)
        {
            ...
            Serial.printf("Slider2 target: %f\n", slider2_target);
            Write_HMI("t2", "txt", roundFloatToString(slider2_target, 1));
            SERVO_MoveSingleAxisAbsPos(ID_2, DEG_TO_POS_2_CONV(roundToNearestHalfDeg(slider2_target - THETA2_OFFSET)), Move_Speed);
        }
        else
        {
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS3") != NULL_POS)
    {
        ...
        if (slider3_target_temp >= -45 && slider3_target_temp <= 45)
        {
            ...
            Serial.printf("Slider3 target: %f\n", slider3_target);
            Write_HMI("t3", "txt", roundFloatToString(slider3_target, 1));
            SERVO_MoveSingleAxisAbsPos(ID_3, DEG_TO_POS_3_CONV(roundToNearestHalfDeg(slider3_target)), Move_Speed);
        }
        else
        {
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS4") != NULL_POS)
    {
        ...
        if (slider4_target_temp >= -90 && slider4_target_temp <= 90)
        {
            ...
            Serial.printf("Slider4 target: %f\n", slider4_target);
            Write_HMI("t4", "txt", roundFloatToString(slider4_target, 1));
            SERVO_MoveSingleAxisAbsPos(ID_4, DEG_TO_POS_4_CONV(roundToNearestHalfDeg(slider4_target)), Move_Speed);
        }
        else
        {
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS5") != NULL_POS)
    {
        ...
        if (slider5_target_temp >= -125 && slider5_target_temp <= 125)
        {
            ...
            Serial.printf("Slider5 target: %f\n", slider5_target);
            Write_HMI("t5", "txt", roundFloatToString(slider5_target, 1));
            SERVO_MoveSingleAxisAbsPos(ID_5, DEG_TO_POS_5_CONV(roundToNearestHalfDeg(slider5_target)), Move_Speed * 5);
        }
        else
        {
    ...
    if (HMI_BUFFER_DATA.indexOf("SLS6") != NULL_POS)
    {
        ...
        if (slider6_target_temp >= -180 && slider6_target_temp <= 180)
        {
            ...
            Serial.printf("Slider6 target: %f\n", slider6_target);
            Write_HMI("t6", "txt", roundFloatToString(slider6_target, 1));
            SERVO_MoveSingleAxisAbsPos(ID_6, DEG_TO_POS_6_CONV(roundToNearestHalfDeg(slider6_target)), Move_Speed);
        }
        else
        {
...
void Check_And_Run_Robot_IK_ENC()
{
    ...
    if (HMI_BUFFER_DATA.indexOf("SIK") != NULL_POS)
    {
        ...
        Serial.printf("EEx_Target: %f, EEy_Target: %f, EEz_Target: %f, EE Selected: %s, Time Target: %d\n",
                      EEx_Target, EEy_Target, EEz_Target, splitString(HMI_BUFFER_DATA, ";", 4), Run_Time_Target);
        ...
        if (IK_Result.indexOf("OK") != NULL_POS)
        {
            ...
            Speed_2 = SPEED_SERVO_2(Theta2_IK_READY - 90, theta2_old - 90, Run_Time_Target);
            ...
            Serial.printf("THETA READY: T1: %f, T2: %f, T3: %f, T4: %f, T5: %f, T6: %f\n",
                          Theta1_IK_READY, Theta2_IK_READY, Theta3_IK_READY, Theta4_IK_READY, Theta5_IK_READY, Theta6_IK_READY);
    ...
    Read_HMI();
    if ((HMI_BUFFER_DATA.indexOf("RUN") != NULL_POS || digitalRead(GREEN_BUTTON) == 0) && EN_READY == true)
    {
        SERVO_MoveSingleAxisAbsPos(ID_1, DEG_TO_POS_1_CONV(Theta1_IK_READY), Speed_1);
        delayMicroseconds(1500);
        SERVO_MoveSingleAxisAbsPos(ID_2, DEG_TO_POS_2_CONV(Theta2_IK_READY - 90), Speed_2);
        delayMicroseconds(1500);
        ...
        Serial.printf("THETA UPDATE: T1: %f, T2: %f, T3: %f, T4: %f, T5: %f, T6: %f\n",
                      GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
        EN_READY = false;
        DONE_ALM = true;
    }
    if ((HMI_BUFFER_DATA.indexOf("STOP") != NULL_POS || digitalRead(RED_BUTTON) == 0))
    {
        SERVO_AllMoveStop();
    ...
    if (HMI_BUFFER_DATA.indexOf("SIK") != NULL_POS)
    {
        ...
        if (IK_Result.indexOf("OK") != NULL_POS)
        {
            ...
            do
            {
                ...
                Write_HMI("t6", "txt", roundFloatToString(GET_THETA_6(), 1));
                FK_Jog = Forward_Kinematics(GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
                Write_HMI("xfk", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(0, 3).value() * 10) / 10, 1));
                Write_HMI("yfk", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(1, 3).value() * 10) / 10, 1));
                Write_HMI("zfk", "txt", roundFloatToString(round(FK_Jog.block<1, 1>(2, 3).value() * 10) / 10, 1));

                EXIT1 = (GET_SPEED_1() == 0 && GET_SPEED_2() == 0 && GET_SPEED_3() == 0 && GET_SPEED_4() == 0 && GET_SPEED_5() == 0 && GET_SPEED_6() == 0);
                EXIT2 = round(GET_THETA_1()) == round(Theta1_IK_READY) && round(GET_THETA_2() + THETA2_OFFSET) == round(Theta2_IK_READY) && round(GET_THETA_3()) == round(Theta3_IK_READY);
                EXIT3 = round(GET_THETA_4()) == round(Theta4_IK_READY) && round(GET_THETA_5()) == round(Theta5_IK_READY) && round(GET_THETA_6()) == round(Theta6_IK_READY);
...
void Kinematic_Slider_Mode_Page()
{
    ...
    // Serial.printf("ST2: %ld\n", map(GET_THETA_2() + THETA2_OFFSET, 0, 135, 0, 135));
...
void Kinematic_Encoder_Mode_Page()
{
    ...
    if (Encoder_Pulse_Resolution != 0)
    {
        ...
        {
            SERVO_MoveSingleAxisAbsPos(ID_1, DEG_TO_POS_1_CONV(roundToNearestHalfDeg(GET_THETA_1() + Encoder_Pulse_Resolution)), 50000);
            lastPosition = Teach_Pendant_Encoder.getCount();
            // Write_HMI("t1", "txt", roundFloatToString(GET_THETA_1()));
        }
        else if (Theta_Selection == "ST2")
        {
            SERVO_MoveSingleAxisAbsPos(ID_2, DEG_TO_POS_2_CONV(roundToNearestHalfDeg(GET_THETA_2() + Encoder_Pulse_Resolution)), 50000);
            lastPosition = Teach_Pendant_Encoder.getCount();
            // Write_HMI("t2", "txt", roundFloatToString(GET_THETA_2()));
        }
        else if (Theta_Selection == "ST3")
        {
            SERVO_MoveSingleAxisAbsPos(ID_3, DEG_TO_POS_3_CONV(roundToNearestHalfDeg(GET_THETA_3() + Encoder_Pulse_Resolution)), 50000);
            lastPosition = Teach_Pendant_Encoder.getCount();
            // Write_HMI("t3", "txt", roundFloatToString(GET_THETA_3()));
        }
        else if (Theta_Selection == "ST4")
        {
            SERVO_MoveSingleAxisAbsPos(ID_4, DEG_TO_POS_4_CONV(roundToNearestHalfDeg(GET_THETA_4() + Encoder_Pulse_Resolution)), 100000);
            lastPosition = Teach_Pendant_Encoder.getCount();
            // Write_HMI("t4", "txt", roundFloatToString(GET_THETA_4()));
        }
        else if (Theta_Selection == "ST5")
        {
            SERVO_MoveSingleAxisAbsPos(ID_5, DEG_TO_POS_5_CONV(roundToNearestHalfDeg(GET_THETA_5() + Encoder_Pulse_Resolution)), 100000);
            lastPosition = Teach_Pendant_Encoder.getCount();
            // Write_HMI("t5", "txt", roundFloatToString(GET_THETA_5()));
        }
        else if (Theta_Selection == "ST6")
        {
            SERVO_MoveSingleAxisAbsPos(ID_6, DEG_TO_POS_6_CONV(roundToNearestHalfDeg(GET_THETA_6() + Encoder_Pulse_Resolution)), 50000);
            lastPosition = Teach_Pendant_Encoder.getCount();
...
void Select_EE_Orientation_Page()
{
    ...
    {
        R_0_6_Temp << FK_Jog.block<1, 1>(0, 0).value(), FK_Jog.block<1, 1>(0, 1).value(), FK_Jog.block<1, 1>(0, 2),
            FK_Jog.block<1, 1>(1, 0).value(), FK_Jog.block<1, 1>(1, 1).value(), FK_Jog.block<1, 1>(1, 2).value(),
            FK_Jog.block<1, 1>(2, 0), FK_Jog.block<1, 1>(2, 1), FK_Jog.block<1, 1>(2, 2).value();
        Write_HMI("almEEC", "txt", "EEC Created!");
    }
}
void Run_Robot_To_Target_Auto_Trajectory_Planning(String move_Mode)
{
    unsigned int Time_Move_Change = 10;
    float Distance_Diff = 0;
    if (EE_Selection == "EE3" || EE_Selection == "EE4" || EE4_Option == true)
        FIND_NEAREST_SOLUTION = true;
    else
        FIND_NEAREST_SOLUTION = false;
    if (EE4_Option == true)
    {
        Serial.println("EE4 Option is TRUE");
    }

    FK_Jog = Forward_Kinematics(GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
    bool X_Move = round(FK_Jog.block<1, 1>(0, 3).value()) == round(Row_Data.EEx_Value);
    bool Y_Move = round(FK_Jog.block<1, 1>(1, 3).value()) == round(Row_Data.EEy_Value);
    bool Z_Move = round(FK_Jog.block<1, 1>(2, 3).value()) == round(Row_Data.EEz_Value);
    bool X_Translational = Y_Move && Z_Move;
    bool Y_Translational = X_Move && Z_Move;
    bool Z_Translational = X_Move && Y_Move;

    if (X_Translational || Y_Translational || Z_Translational)
    {
        if (X_Translational)
            Distance_Diff = abs(round(FK_Jog.block<1, 1>(0, 3).value()) - round(Row_Data.EEx_Value));
        else if (Y_Translational)
            Distance_Diff = abs(round(FK_Jog.block<1, 1>(1, 3).value()) - round(Row_Data.EEy_Value));
        else if (Z_Translational)
            Distance_Diff = abs(round(FK_Jog.block<1, 1>(2, 3).value()) - round(Row_Data.EEz_Value));
        Trajectory.Total_Index = Distance_Diff * 2;

        Trajectory.ACC_Time = 5;
        Trajectory.DEC_Time = 5;

        SET_ALL_ACCELERATION_TIME(Trajectory.ACC_Time);
        SET_ALL_DECELERATION_TIME(Trajectory.DEC_Time);
        for (int Value_Index = 0; Value_Index < Trajectory.Total_Index + 1; Value_Index++)
        {
            if (X_Translational && (round(Row_Data.EEx_Value) > round(FK_Jog.block<1, 1>(0, 3).value())))
                EE_Target_Matrix << round(FK_Jog.block<1, 1>(0, 3).value()) + 0.5, EEy_Target, EEz_Target;
            else if (X_Translational && (round(Row_Data.EEx_Value) < round(FK_Jog.block<1, 1>(0, 3).value())))
                EE_Target_Matrix << round(FK_Jog.block<1, 1>(0, 3).value()) - 0.5, EEy_Target, EEz_Target;

            if (Y_Translational && (round(Row_Data.EEy_Value) > round(FK_Jog.block<1, 1>(1, 3).value())))
                EE_Target_Matrix << EEx_Target, round(FK_Jog.block<1, 1>(1, 3).value()) + 0.5, EEz_Target;
            else if (Y_Translational && (round(Row_Data.EEy_Value) < round(FK_Jog.block<1, 1>(1, 3).value())))
                EE_Target_Matrix << EEx_Target, round(FK_Jog.block<1, 1>(1, 3).value()) - 0.5, EEz_Target;

            if (Z_Translational && (round(Row_Data.EEz_Value) > round(FK_Jog.block<1, 1>(2, 3).value())))
                EE_Target_Matrix << EEx_Target, EEy_Target, round(FK_Jog.block<1, 1>(2, 3).value()) + 0.5;
            else if (Z_Translational && (round(Row_Data.EEz_Value) < round(FK_Jog.block<1, 1>(2, 3).value())))
                EE_Target_Matrix << EEx_Target, EEy_Target, round(FK_Jog.block<1, 1>(2, 3).value()) - 0.5;
            ...
            if (IK_Result.indexOf("OK") != NULL_POS)
            {
                ...
                Trajectory.theta[0][Value_Index] = Theta1_IK_READY;
                Trajectory.theta[1][Value_Index] = Theta2_IK_READY;
                Trajectory.theta[2][Value_Index] = Theta3_IK_READY;
                Trajectory.theta[3][Value_Index] = Theta4_IK_READY;
                Trajectory.theta[4][Value_Index] = Theta5_IK_READY;
                Trajectory.theta[5][Value_Index] = Theta6_IK_READY;

                unsigned int moveTime = Row_Data.moveTimeValue / Trajectory.Total_Index;
                Trajectory.speed[0][Value_Index] = SPEED_SERVO_1(Theta1_IK_READY, GET_THETA_1(), moveTime);
                Trajectory.speed[1][Value_Index] = SPEED_SERVO_2(Theta2_IK_READY - THETA2_OFFSET, GET_THETA_2(), moveTime);
                Trajectory.speed[2][Value_Index] = SPEED_SERVO_3(Theta3_IK_READY, GET_THETA_3(), moveTime);
                Trajectory.speed[3][Value_Index] = SPEED_SERVO_4(Theta4_IK_READY, GET_THETA_4(), moveTime);
                Trajectory.speed[4][Value_Index] = SPEED_SERVO_5(Theta5_IK_READY, GET_THETA_5(), moveTime);
                Trajectory.speed[5][Value_Index] = SPEED_SERVO_6(Theta6_IK_READY, GET_THETA_6(), moveTime);
            }
            else
                return;
        }

        for (int Index = 0; Index < Trajectory.Total_Index + 1; Index++)
        {
            SERVO_MoveSingleAxisAbsPos(ID_1, DEG_TO_POS_1_CONV(Trajectory.theta[0][Index]), Trajectory.speed[0][Index]);
            delayMicroseconds(1500);
            SERVO_MoveSingleAxisAbsPos(ID_2, DEG_TO_POS_2_CONV(Trajectory.theta[1][Index]), Trajectory.speed[1][Index]);
            delayMicroseconds(1500);
            SERVO_MoveSingleAxisAbsPos(ID_3, DEG_TO_POS_3_CONV(Trajectory.theta[2][Index]), Trajectory.speed[2][Index]);
            delayMicroseconds(1500);
            SERVO_MoveSingleAxisAbsPos(ID_4, DEG_TO_POS_4_CONV(Trajectory.theta[3][Index]), Trajectory.speed[3][Index]);
            delayMicroseconds(1500);
            SERVO_MoveSingleAxisAbsPos(ID_5, DEG_TO_POS_5_CONV(Trajectory.theta[4][Index]), Trajectory.speed[4][Index]);
            delayMicroseconds(1500);
            SERVO_MoveSingleAxisAbsPos(ID_6, DEG_TO_POS_6_CONV(Trajectory.theta[5][Index]), Trajectory.speed[5][Index]);
            delayMicroseconds(1500);
            delayMicroseconds(static_cast<unsigned long>(Row_Data.moveTimeValue / Trajectory.Total_Index) * 1000000UL - 9000UL);
        }
    ...
    else
    {
        ...
        if (IK_Result.indexOf("OK") != NULL_POS)
        {
            ...
            if (move_Mode == "EEMOVE")
            {
                ...
                Time_Move_Change = Row_Data.moveTimeValue;
            else
                Time_Move_Change = 10;
        }
        else
        ...
        Speed_2 = SPEED_SERVO_2(Theta2_IK_READY - 90, theta2_old - 90, Time_Move_Change);
...
void Move_Change_EE_Orientatation_Base_Mode_Page()
{
    ...
    Run_Robot_To_Target_Auto_Trajectory_Planning("EEMOVE");
}
void Check_Move_Change_EE_Orientation_Base_Mode()
{
    ...
    {
        bool EN_MOVE = (GET_SPEED_1() == 0 && GET_SPEED_2() == 0 && GET_SPEED_3() == 0 && GET_SPEED_4() == 0 && GET_SPEED_5() == 0 && GET_SPEED_6() == 0);
        if (EN_MOVE)
        {
            R_0_6_Temp << 1, 0, 0,
                0, -1, 0,
...
void Base_Mode_Page()
{
    ...
    if (Axis_Resolution >= -50 && Axis_Resolution != 0 && Axis_Resolution <= 50)
    {
        ...
        if (EN_MOVE)
        {
            ...
            {
                FK_Jog = Forward_Kinematics(GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
                EE_Target_Matrix << round(FK_Jog.block<1, 1>(0, 3).value() + Axis_Resolution),
                    round(FK_Jog.block<1, 1>(1, 3).value()),
                    round(FK_Jog.block<1, 1>(2, 3).value());
                if (EE_Selection == "EE4")
                    EE4_Option = true;
                Run_Robot_To_Target_Auto_Trajectory_Planning("AXISMOVE");
                lastPosition = Teach_Pendant_Encoder.getCount();
            }
...
{
    String pathFile = "/P" + String(rowNo) + "T" + String(Table_No) + ".txt";
    String markAndRowInfo = "1;" + String(Row_Data.rowNo) + ";" + Row_Data.posName + ";";
    String positionInfo = String(Row_Data.EEx_Value) + ";" + String(Row_Data.EEy_Value) + ";" + String(Row_Data.EEz_Value) + ";" + Row_Data.EE_Orientation + ";";
    String timeAndRelayState = String(Row_Data.moveTimeValue) + ";" + String(Row_Data.waitTimeValue) + ";" + Row_Data.out1State + ";" + Row_Data.out2State + ";";
    String R_0_6_EEC = "";
    if (Row_Data.EE_Orientation == "EEC")
    {
        String R_0_6_EEC_1 = String(Row_Data.EEC_R06_Matrix.r11) + ";" + String(Row_Data.EEC_R06_Matrix.r12) + ";" + String(Row_Data.EEC_R06_Matrix.r13) + ";";
        String R_0_6_EEC_2 = String(Row_Data.EEC_R06_Matrix.r21) + ";" + String(Row_Data.EEC_R06_Matrix.r22) + ";" + String(Row_Data.EEC_R06_Matrix.r23) + ";";
        String R_0_6_EEC_3 = String(Row_Data.EEC_R06_Matrix.r31) + ";" + String(Row_Data.EEC_R06_Matrix.r32) + ";" + String(Row_Data.EEC_R06_Matrix.r33) + ";";
        R_0_6_EEC = R_0_6_EEC_1 + R_0_6_EEC_2 + R_0_6_EEC_3;
    }
    else
        R_0_6_EEC = "";
...
{
    Row_Data.posName = splitString(HMI_BUFFER_DATA, ";", 1);
    Row_Data.EEx_Value = splitString(HMI_BUFFER_DATA, ";", 2).toFloat();
    Row_Data.EEy_Value = splitString(HMI_BUFFER_DATA, ";", 3).toFloat();
    Row_Data.EEz_Value = splitString(HMI_BUFFER_DATA, ";", 4).toFloat();
    Row_Data.EE_Orientation = splitString(HMI_BUFFER_DATA, ";", 5);
    Row_Data.moveTimeValue = splitString(HMI_BUFFER_DATA, ";", 6).toInt();
    Row_Data.waitTimeValue = splitString(HMI_BUFFER_DATA, ";", 7).toInt();
    Row_Data.out1State = splitString(HMI_BUFFER_DATA, ";", 8);
    Row_Data.out2State = splitString(HMI_BUFFER_DATA, ";", 9);
    if (Row_Data.EE_Orientation == "EEC")
    {
        Row_Data.EEC_R06_Matrix.r11 = R_0_6_Temp.block<1, 1>(0, 0).value();
        Row_Data.EEC_R06_Matrix.r12 = R_0_6_Temp.block<1, 1>(0, 1).value();
        Row_Data.EEC_R06_Matrix.r13 = R_0_6_Temp.block<1, 1>(0, 2).value();

        Row_Data.EEC_R06_Matrix.r21 = R_0_6_Temp.block<1, 1>(1, 0).value();
        Row_Data.EEC_R06_Matrix.r22 = R_0_6_Temp.block<1, 1>(1, 1).value();
        Row_Data.EEC_R06_Matrix.r23 = R_0_6_Temp.block<1, 1>(1, 2).value();

        Row_Data.EEC_R06_Matrix.r31 = R_0_6_Temp.block<1, 1>(2, 0).value();
        Row_Data.EEC_R06_Matrix.r32 = R_0_6_Temp.block<1, 1>(2, 1).value();
        Row_Data.EEC_R06_Matrix.r33 = R_0_6_Temp.block<1, 1>(2, 2).value();
    }
}
void Add_SD_Data_To_Struct(String rowData)
{
    // Content Format: Has_Data;RowNo;EEx;EEy;EEz;EE_Orientation;MoveTime;WaitTime;Output1_State;Output2_State;r11;r12;r13;r21;r22;r23;r31;r32;r33;
    // Example: 1;9;385.0;165.3;419.5;EEC;2;1;1;0;0.92;-0.03;0.38;-0.06;-1.0;0.07;0.39;-0.09;-0.92;
    Row_Data.rowNo = splitString(rowData, ";", 1).toInt();
    Row_Data.posName = splitString(rowData, ";", 2);
    Row_Data.EEx_Value = splitString(rowData, ";", 3).toFloat();
    Row_Data.EEy_Value = splitString(rowData, ";", 4).toFloat();
    Row_Data.EEz_Value = splitString(rowData, ";", 5).toFloat();
    Row_Data.EE_Orientation = splitString(rowData, ";", 6);
    Row_Data.moveTimeValue = splitString(rowData, ";", 7).toInt();
    Row_Data.waitTimeValue = splitString(rowData, ";", 8).toInt();
    Row_Data.out1State = splitString(rowData, ";", 9);
    Row_Data.out2State = splitString(rowData, ";", 10);
    if (Row_Data.EE_Orientation == "EEC")
    {
        Row_Data.EEC_R06_Matrix.r11 = splitString(rowData, ";", 11).toFloat();
        Row_Data.EEC_R06_Matrix.r12 = splitString(rowData, ";", 12).toFloat();
        Row_Data.EEC_R06_Matrix.r13 = splitString(rowData, ";", 13).toFloat();

        Row_Data.EEC_R06_Matrix.r21 = splitString(rowData, ";", 14).toFloat();
        Row_Data.EEC_R06_Matrix.r22 = splitString(rowData, ";", 15).toFloat();
        Row_Data.EEC_R06_Matrix.r23 = splitString(rowData, ";", 16).toFloat();

        Row_Data.EEC_R06_Matrix.r31 = splitString(rowData, ";", 17).toFloat();
        Row_Data.EEC_R06_Matrix.r32 = splitString(rowData, ";", 18).toFloat();
        Row_Data.EEC_R06_Matrix.r33 = splitString(rowData, ";", 19).toFloat();
    }
}
void Edit_Data_To_Struct()
{
    Row_Data.rowNo = splitString(HMI_BUFFER_DATA, ";", 1).toInt();
    Row_Data.posName = splitString(HMI_BUFFER_DATA, ";", 2);
    Row_Data.EEx_Value = splitString(HMI_BUFFER_DATA, ";", 3).toFloat();
    Row_Data.EEy_Value = splitString(HMI_BUFFER_DATA, ";", 4).toFloat();
    Row_Data.EEz_Value = splitString(HMI_BUFFER_DATA, ";", 5).toFloat();
    Row_Data.EE_Orientation = splitString(HMI_BUFFER_DATA, ";", 6);
    Row_Data.moveTimeValue = splitString(HMI_BUFFER_DATA, ";", 7).toInt();
    Row_Data.waitTimeValue = splitString(HMI_BUFFER_DATA, ";", 8).toInt();
    Row_Data.out1State = splitString(HMI_BUFFER_DATA, ";", 9);
    Row_Data.out2State = splitString(HMI_BUFFER_DATA, ";", 10);
    if (Row_Data.EE_Orientation == "EEC")
    {
        Row_Data.EEC_R06_Matrix.r11 = R_0_6_Temp.block<1, 1>(0, 0).value();
        Row_Data.EEC_R06_Matrix.r12 = R_0_6_Temp.block<1, 1>(0, 1).value();
...
{
    unsigned int rowNo = (unsigned int)(row_update - ((row_begin + 9) / 10 - 1) * 10);
    Serial.printf("Update Data Row: %d - [Row_Update: %d]\n", rowNo, row_update);
    Write_HMI("n" + String(rowNo), "txt", String(row_update));
    Write_HMI("p" + String(rowNo), "txt", Row_Data.posName);
    Write_HMI("x" + String(rowNo), "txt", roundFloatToString(Row_Data.EEx_Value, 1));
    Write_HMI("y" + String(rowNo), "txt", roundFloatToString(Row_Data.EEy_Value, 1));
    Write_HMI("z" + String(rowNo), "txt", roundFloatToString(Row_Data.EEz_Value, 1));
    Write_HMI("e" + String(rowNo), "txt", Row_Data.EE_Orientation);
    Write_HMI("m" + String(rowNo), "txt", String(Row_Data.moveTimeValue));
    Write_HMI("w" + String(rowNo), "txt", String(Row_Data.waitTimeValue));
    Write_HMI("o1" + String(rowNo), "pco", Row_Data.out1State == "ON" ? 2016 : 63488);
    Write_HMI("o1" + String(rowNo), "txt", Row_Data.out1State);
    Write_HMI("o2" + String(rowNo), "pco", Row_Data.out2State == "ON" ? 2016 : 63488);
    Write_HMI("o2" + String(rowNo), "txt", Row_Data.out2State);
}
...
void Update_Data_Table(unsigned int row_begin, unsigned int row_end)
{
    ...
    {
        String pathFile = "/P" + String(row) + "T" + String(Table_No) + ".txt";
        ...
        if (splitString(contentReturn, ";", 0) == "1")
        {
            ...
            Update_Row_Data_HMI(Row_Data.rowNo);
        }
        else
        {
            Update_Row_Null_HMI(row);
        }
    }
}
...
{
    unsigned int rowNo = (unsigned int)(row_update - ((row_begin + 9) / 10 - 1) * 10);
    Serial.printf("Mark Row: %d - [Row_Update: %d]\n", rowNo, row_update);
...
{
    Serial.println("Clear Data Table");
    Update_Data_Table(row_begin, row_begin + 9);
    for (int i = 1; i <= sizeof(Row_Data.Mark_Array) / sizeof(Row_Data.Mark_Array[1]); i++)
    {
        Serial.printf("Clear Row: %d\n", i);
        String pathFile = "/P" + String(i) + "T" + String(Table_No) + ".txt";
        String contentReturn = readFile(SD, pathFile.c_str());
        String contentWrite = " ";

        Serial.printf("PathFile: %s\n", pathFile.c_str());
        Serial.printf("contentReturn: %s\n", contentReturn.c_str());

        if (splitString(contentReturn, ";", 0) == "1")
        {
            bool writeFileStatus = writeFile(SD, pathFile.c_str(), contentWrite.c_str());
            Update_Row_Null_HMI(i);
            if (writeFileStatus == true)
            {
                // Serial.printf("Cleared Row: %d\n", i);
                Mark_Row(i, true);
                ENABLE_BUZZER(75);
                Mark_Row(i, false);
            }
            else
                Alarm_HMI("Clear Failed!");
        }
        else
        {
            Serial.println("Clear Done");
            Alarm_HMI("Clear Successfully!");
            ENABLE_BUZZER(300);
            return;
        }
    }
...
void Run_Robot_Position_Table()
{
    ...
    for (int i = 1; i <= sizeof(Row_Data.Mark_Array) / sizeof(Row_Data.Mark_Array[1]); i++)
    {
        String pathFile_RUN = "/P" + String(i) + "T" + String(Table_No) + ".txt";
        ...
        if (splitString(contentReturn_RUN, ";", 0) == "1")
        {
            ...
            Mark_Row(Row_Data.rowNo, true);
            EE_Target_Matrix << Row_Data.EEx_Value,
                Row_Data.EEy_Value,
                Row_Data.EEz_Value;

            if (Row_Data.EE_Orientation == "EE1")
            {
                Write_HMI("EEOri", "txt", "EE1");
                R_0_6_Temp << 1, 0, 0,
                    0, -1, 0,
                    0, 0, -1;
            }
            else if (Row_Data.EE_Orientation == "EE2")
            {
                Write_HMI("EEOri", "txt", "EE2");
                R_0_6_Temp << 0, 0, 1,
                    0, -1, 0,
                    1, 0, 0;
            }
            else if (Row_Data.EE_Orientation == "EE3")
            {
                Write_HMI("EEOri", "txt", "EE3");
                R_0_6_Temp << 1, 0, 0,
                    0, 0, 1,
                    0, -1, 0;
            }
            else if (Row_Data.EE_Orientation == "EE4")
            {
                Write_HMI("EEOri", "txt", "EE4");
                R_0_6_Temp << -1, 0, 0,
                    0, 0, -1,
                    0, -1, 0;
                EE4_Option = true;
            ...
            else if (Row_Data.EE_Orientation == "EEC")
            {
                Write_HMI("EEOri", "txt", "EEC");
                R_0_6_Temp << Row_Data.EEC_R06_Matrix.r11, Row_Data.EEC_R06_Matrix.r12, Row_Data.EEC_R06_Matrix.r13,
                    Row_Data.EEC_R06_Matrix.r21, Row_Data.EEC_R06_Matrix.r22, Row_Data.EEC_R06_Matrix.r23,
                    Row_Data.EEC_R06_Matrix.r31, Row_Data.EEC_R06_Matrix.r32, Row_Data.EEC_R06_Matrix.r33;
            }
            Run_Robot_To_Target_Auto_Trajectory_Planning("ROWMOVE");
            bool EXIT1 = false, EXIT2 = false, EXIT3 = false;
            do
            {
                FK_Jog = Forward_Kinematics(GET_THETA_1(), GET_THETA_2() + THETA2_OFFSET, GET_THETA_3(), GET_THETA_4(), GET_THETA_5(), GET_THETA_6());
                ...
                // EXIT2 = round(GET_THETA_1()) == round(Theta1_IK_READY) && round(GET_THETA_2() + THETA2_OFFSET) == round(Theta2_IK_READY) && round(GET_THETA_3()) == round(Theta3_IK_READY);
                // EXIT3 = round(GET_THETA_4()) == round(Theta4_IK_READY) && round(GET_THETA_5()) == round(Theta5_IK_READY) && round(GET_THETA_6()) == round(Theta6_IK_READY);
                if (digitalRead(RED_BUTTON) == 0 && digitalRead(GREEN_BUTTON) == 0)
                {
                    Mark_Row(Row_Data.rowNo, false);
                    return;
                }
            } while (!(EXIT1));
            if (Row_Data.out1State == "ON")
                digitalWrite(RELAY_5, HIGH);
            else if (Row_Data.out1State == "OFF")
                digitalWrite(RELAY_5, LOW);
            if (Row_Data.out2State == "ON")
                digitalWrite(RELAY_6, HIGH);
            else if (Row_Data.out2State == "OFF")
                digitalWrite(RELAY_6, LOW);
            delay(Row_Data.waitTimeValue * 1000);
            Mark_Row(Row_Data.rowNo, false);
        }
        else
        {
...
void Position_Table_Page()
{
    ...
    if (HMI_BUFFER_DATA.indexOf("ADDR") != NULL_POS)
    {
        for (int i = 1; i <= sizeof(Row_Data.Mark_Array) / sizeof(Row_Data.Mark_Array[1]); i++)
        {
            ...
            if (splitString(contentReturn, ";", 0) != "1")
            {
                ...
                Row_Data.Mark_Array[i] = 1;
                Row_Data.rowNo = i;
                Add_HMI_Data_To_Struct();
                Add_Row_Data_To_File(Row_Data.rowNo);
                // Write_HMI("tablename", "txt", Table_Name);
                // Write_HMI("pageno", "val", (unsigned int)((row_begin + 9) / 10));
                // Update_Data_Table(row_begin, row_begin + 9);
                // Update_Row_Data_HMI(Row_Data.rowNo);
    ...
    else if (HMI_BUFFER_DATA.indexOf("CLRTB") != NULL_POS)
    {
        Clear_Data_Table();
        clearBufferData();
    }
...
void TCP_Led()
{
    ...
    {
        if ((unsigned long)millis() - previous_Time_D >= interval / 2)
        {
            D_State = !D_State;
            Write_HMI("dled", "bco", (D_State ? 64512 : 8518));
            previous_Time_D = millis();
        }
    }
}
void LINK_Led()
{
    if (Ethernet.linkStatus() == LinkON)
    {
        ...
        {
            L_State = !L_State;
            Write_HMI("cled", "bco", (L_State ? 2016 : 8518));
    ...
    else
    {
        ...
        {
            L_State = !L_State;
            Write_HMI("cled", "bco", (L_State ? 63488 : 8518));
            previous_Time = millis();
        }
        Write_HMI("dled", "bco", 8518);
    }
}
void Run_Robot_TCP_Mode(int16_t table_target)
{
    Modbus.task();
    Modbus.Hreg(HREG1_DATA_WRITE, 1);
    // Write_HMI("pageno", "val", (unsigned int)((row_begin + 9) / 10));
    // Update_Data_Table(row_begin, row_begin + 9);
    for (int i = 1; i <= sizeof(Row_Data.Mark_Array) / sizeof(Row_Data.Mark_Array[1]); i++)
    {
        String pathFile_RUN = "/P" + String(i) + "T" + String(table_target) + ".txt";
        String contentReturn_RUN = readFile(SD, pathFile_RUN.c_str());
        ...
        if (splitString(contentReturn_RUN, ";", 0) == "1")
        {
            ...
            {
                Modbus.task();
                TCP_DATA = (int16_t)Modbus.Hreg(HREG2_DATA_READ);
                if (TCP_DATA == TCP_TASK_STOP && Run_Mark != TCP_TASK_STOP) // STOP
                {
                    SERVO_MoveStop(ID_1);
                    SERVO_MoveStop(ID_2);
                    SERVO_MoveStop(ID_3);
                    ...
                    SERVO_MoveStop(ID_5);
                    SERVO_MoveStop(ID_6);
                    Run_Mark = TCP_TASK_STOP;
                    return;
                }
                // Serial.printf("i = %d\n", i);
                if (i > 5)
                {
                    if (table_target == 5 || table_target == 6 || table_target == 7 || table_target == 8 || table_target == 9 || table_target == 10)
                    {
                        // Serial.printf("table_target = %d\n", table_target);
                        Modbus.task();
                        Modbus.Hreg(HREG1_DATA_WRITE, 0);
                    }
                }
                else
                {
            ...
            unsigned long time_start_wait = millis();
            do
            {
                Modbus.task();
                TCP_DATA = (int16_t)Modbus.Hreg(HREG2_DATA_READ);
                ...
                Write_HMI("out2", "pco", digitalRead(RELAY_6) ? 2016 : 63488);
                Write_HMI("out2", "txt", String((digitalRead(RELAY_6) ? "ON" : "OFF")));
            } while ((unsigned long)millis() - time_start_wait <= Row_Data.waitTimeValue * 1000);
            // Mark_Row(Row_Data.rowNo, false);
        }
        else
        {
...
void Check_Stop_Robot_TCP_Mode()
{
    ...
    do
    {
        ...
        LINK_Led();
        TCP_Led();
        EXIT = (GET_SPEED_1() == 0 && GET_SPEED_2() == 0 && GET_SPEED_3() == 0 && GET_SPEED_4() == 0 && GET_SPEED_5() == 0 && GET_SPEED_6() == 0);
    } while (!EXIT);
    Modbus.task();
    Modbus.Hreg(HREG1_DATA_WRITE, 0);
}
...
{
    Read_HMI();
    if (HMI_BUFFER_DATA.indexOf("ETHC") != NULL_POS)
    {
        ETH_Connect = !ETH_Connect;
        if (ETH_Connect && Ethernet.linkStatus() == LinkON)
        {
            Write_HMI("ethbt", "txt", "Disconnect");
            ETH_Connect = true;
        }
        else
        {
    ...
    if (ETH_Connect == true)
    {
        Modbus.task();
        TCP_DATA = (int16_t)Modbus.Hreg(HREG2_DATA_READ);
        Serial.printf("TCP Data Read: %d\n", TCP_DATA);
        Write_HMI("ethst", "pco", 2016);
        Write_HMI("ethst", "txt", "Connected!");
        if (TCP_DATA == TCP_TASK_POS_1 && Run_Mark != TCP_TASK_POS_1) // POS1
        {
            Run_Robot_TCP_Mode(5);
            Run_Mark = TCP_TASK_POS_1;
        }
        else if (TCP_DATA == TCP_TASK_POS_2 && Run_Mark != TCP_TASK_POS_2) // POS2
        {
            Run_Robot_TCP_Mode(6);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_POS_2;
        }
        else if (TCP_DATA == TCP_TASK_POS_3 && Run_Mark != TCP_TASK_POS_3) // POS3
        {
            Run_Robot_TCP_Mode(7);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_POS_3;
        }
        else if (TCP_DATA == TCP_TASK_POS_4 && Run_Mark != TCP_TASK_POS_4) // POS4
        {
            Run_Robot_TCP_Mode(8);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_POS_4;
        }
        else if (TCP_DATA == TCP_TASK_POS_5 && Run_Mark != TCP_TASK_POS_5) // POS5
        {
            Run_Robot_TCP_Mode(9);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_POS_5;
        }
        else if (TCP_DATA == TCP_TASK_POS_6 && Run_Mark != TCP_TASK_POS_6) // POS6
        {
            Run_Robot_TCP_Mode(10);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_POS_6;
        }
        else if (TCP_DATA == TCP_TASK_POS_TG_1 && Run_Mark != TCP_TASK_POS_TG_1) // POS TG1
        {
            Run_Robot_TCP_Mode(3);
            Run_Mark = TCP_TASK_POS_TG_1;
        }
        else if (TCP_DATA == TCP_TASK_POS_TG_2 && Run_Mark != TCP_TASK_POS_TG_2) // POS TG2
        {
            Run_Robot_TCP_Mode(4);
            Run_Mark = TCP_TASK_POS_TG_2;
        }
        else if (TCP_DATA == TCP_TASK_HOME && Run_Mark != TCP_TASK_HOME) // HOME
        {
            Move_Robot_Fixed_Modes(Fixed_Modes.Home, 3);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_HOME;
        }
        else if (TCP_DATA == TCP_TASK_CALIB && Run_Mark != TCP_TASK_CALIB) // CALIB
        {
            Robot_Calibration();
            Run_Mark = TCP_TASK_CALIB;
        }
        else if (TCP_DATA == TCP_TASK_WAKEUP && Run_Mark != TCP_TASK_WAKEUP) // WAKEUP
        {
            Move_Robot_Fixed_Modes(Fixed_Modes.Wakeup, 3);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_WAKEUP;
        }
        else if (TCP_DATA == TCP_TASK_SLEEP && Run_Mark != TCP_TASK_SLEEP) // SLEEP
        {
            Move_Robot_Fixed_Modes(Fixed_Modes.Sleep, 3);
            Check_Stop_Robot_TCP_Mode();
            Run_Mark = TCP_TASK_SLEEP;
        }
        else if (TCP_DATA == TCP_TASK_ON_SERVO && Run_Mark != TCP_TASK_ON_SERVO) // ON
        {
            ENABLE_ALL_SERVO();
            Run_Mark = TCP_TASK_ON_SERVO;
        }
        else if (TCP_DATA == TCP_TASK_OFF_SERVO && Run_Mark != TCP_TASK_OFF_SERVO) // OFF
        {
            DISABLE_ALL_SERVO();
            Run_Mark = TCP_TASK_OFF_SERVO;
        }
        else if (TCP_DATA == TCP_TASK_STOP && Run_Mark != TCP_TASK_STOP) // STOP
        {
            SERVO_MoveStop(ID_1);
            SERVO_MoveStop(ID_2);
            SERVO_MoveStop(ID_3);
    ...
    LINK_Led();
    Write_HMI("mac", "txt", convertBytesToHexString(MAC_Address));
    Write_HMI("ip", "txt", IP_Address.toString());
    Write_HMI("gateway", "txt", Gateway_Address.toString());
    Write_HMI("subnet", "txt", Subnet_Mask.toString());
    Update_Data_HMI();
}
void Debug_Page()
{
...
void Update_Current_Page_Fcns()
{
    ...
    while (CURRENT_PAGE == "KI_ENC")
    {
        ...
        //               digitalRead(HOME_SENSOR_J1), digitalRead(HOME_SENSOR_J2), digitalRead(HOME_SENSOR_J3), digitalRead(HOME_SENSOR_J4), digitalRead(HOME_SENSOR_J5), digitalRead(HOME_SENSOR_J6));
        Read_HMI();
        Check_Change_Page_Fcns();
    }
    ...
    while (CURRENT_PAGE == "EP")
    {
        // Serial.println("This is the TCP IP Page !");
        TCPIP_Mode_Page();
        Read_HMI();
        Check_Change_Page_Fcns();
    }
...
void SET_ALL_POS_INIT(float theta1_init, float theta2_init, float theta3_init, float theta4_init, float theta5_init, float theta6_init)
{
...
void Initialization_Fcns()
{
    ...
    SET_ALL_POS_INIT(theta1_Init, theta2_Init, theta3_Init, theta4_Init, theta5_Init, theta6_Init);
}
void setup()
{
    /*========== SETUP PINMODE ==========*/
...
/*========== REASSIGN PINS FOR VSPI ==========*/
    ...
    Modbus.config(MAC_Address, IP_Address, DNS_Address, Gateway_Address, Subnet_Mask);

    Modbus.addHreg(HREG1_DATA_WRITE, 0);
    Modbus.addHreg(HREG2_DATA_READ, 0);
}
void loop()
{
