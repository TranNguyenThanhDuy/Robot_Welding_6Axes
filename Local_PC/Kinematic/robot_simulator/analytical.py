...
LINE_MAPPING_SAMPLES_PER_SEGMENT = 40
LINE_MAPPING_LOCK_Z_TO_FIRST = True
CIRCLE_DEFAULT_SAMPLES = 80
CIRCLE_RADIUS = 60.0
...
def build_line_mapping_from_encoder_file(
    ...
    The mapped trajectory can lock all targets to the first waypoint Z plane,
    then inserts straight-line Cartesian samples between each consecutive pair
    of XY waypoints.
    """
    ...
    # Build a polyline from the target waypoints:
    ...
    if LINE_MAPPING_LOCK_Z_TO_FIRST:
        # Only the first waypoint can stay fixed. Later taught waypoints may have
        # different Z, so locking their original joint angles would break the flat plane.
    ...
    # IK chi tinh cho diem can sinh; waypoint that duoc khoa bang goc goc neu khong khoa Z.
    ...
    if LINE_MAPPING_LOCK_Z_TO_FIRST:
        # Keep the exact starting pose. Other waypoints are solved by IK on the flat Z plane.
