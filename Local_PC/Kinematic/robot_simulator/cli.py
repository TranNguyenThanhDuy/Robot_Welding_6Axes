...
def run_slider_ui():
    ...
    def run_circle_mapping(_event):
        ...
        try:
            ...
            reference_pose = Forward_Kinematics(
                reference_angles[0],
                reference_angles[1] + THETA2_OFFSET,
                reference_angles[2],
                reference_angles[3],
                reference_angles[4],
                reference_angles[5],
            )
            center_x, center_y, z_height = reference_pose[:3, 3]
            mapping = export_circle_mapping_xy(
                ...
                fixed_rotation=reference_pose[:3, :3],
            )
        except Exception as exc:
...
def run_line_mapping_cli(file_path):
    ...
    try:
        ...
        print(f"Line mapping failed: {format_mapping_error(exc, 'Line Map')}")
        return 1
    ...
    if "rotation_strategy" in mapping:
        print(f"Rotation strategy: {mapping['rotation_strategy']}")
    return 0


def run_circle_mapping_cli(file_path):
    if not file_path:
        print("Missing circle-map reference pose file path.")
        return 1
    if not os.path.isfile(file_path):
        print(f"Input file not found: {file_path}")
        return 1
    ...
    try:
        ...
        print(f"Circle mapping failed: {format_mapping_error(exc, 'Circle')}")
        return 1
