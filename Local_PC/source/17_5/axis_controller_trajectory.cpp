...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    for (size_t i = 0; i < steps; ++i) {
        ...
        AxisVelocities velocities = computeSegmentVelocities(previous, target, segment_period_us);
...
bool AxisController::goFromFile(const std::string& filename)
{
    AxisVectors mappedPaths;
    double fileSegmentSeconds = -1.0;
    double fileMoveTimeSeconds = -1.0;
    if (!readMappedPositionFile(filename, mappedPaths, fileSegmentSeconds, fileMoveTimeSeconds)) {
        return false;
    }

    AxisStatuses statuses{};
    if (!readAxisStatuses(statuses) || !allServoOn(statuses)) {
        std::cout << "Some servos are OFF. Turn them ON before trajectory run." << std::endl;
        return false;
    }

    AxisPositions initialPos{};
    if (!readActualPositions(initialPos)) {
        std::cout << "Failed to read actual positions before trajectory planning." << std::endl;
        return false;
    }

    AxisVectors planInput;
    for (size_t axis = 0; axis < AXIS_COUNT; ++axis) {
        planInput[axis].push_back(initialPos[axis]);
        for (int pos : mappedPaths[axis]) {
            planInput[axis].push_back(pos);
        }
    }

    TrajectoryPlannerConfig config;
    if (fileSegmentSeconds > 0.0) {
        config.segment_seconds = fileSegmentSeconds;
        std::cout << "Segment timing from file header: "
                  << fileSegmentSeconds << " s/step." << std::endl;
    } else if (fileMoveTimeSeconds > 0.0 && mappedPaths[0].size() > 1) {
        config.segment_seconds =
            fileMoveTimeSeconds / static_cast<double>(mappedPaths[0].size() - 1);
        std::cout << "Segment timing from moveTimeValue/Total_Index: "
                  << config.segment_seconds << " s/step." << std::endl;
    } else {
        config.segment_seconds = kDefaultSegmentSeconds;
        std::cout << "No segment timing header. Using default "
                  << kDefaultSegmentSeconds << " s/step." << std::endl;
    }

    Trajectory trajectory = planTrajectory(planInput, config);
    if (trajectory.empty()) {
        std::cout << "Trajectory planner produced no points." << std::endl;
        return false;
    }

    const std::string debugTrajectoryFile = filename + ".trajectory.txt";
    writeTrajectoryFile(debugTrajectoryFile, trajectory);
    std::cout << "Planned trajectory points: " << trajectory.size()
              << ". Debug file: " << debugTrajectoryFile << std::endl;

    const auto command_window =
        std::chrono::duration_cast<std::chrono::microseconds>(kCommandSpacing * AXIS_COUNT);
    const auto segment_period_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::duration<double>(config.segment_seconds));
    if (segment_period_us < command_window) {
        std::cout << "Segment period is shorter than command window. Requested "
                  << segment_period_us.count() << " us, minimum command window "
                  << command_window.count() << " us." << std::endl;
    ...
    setOutputSignal(outAxis, outMask, true);
    std::cout << "Welding Output ON. Trajectory streaming started." << std::endl;
    ...
    auto finishRun = [&](bool ok) {
        ...
        setOutputSignal(outAxis, outMask, false);
        if (ok) {
            std::cout << "Welding Output OFF. Trajectory finished." << std::endl;
        }
        return ok;
    };

    AxisPositions previous = initialPos;
    AxisBools axisStreamingStarted{};
    axisStreamingStarted.fill(false);
    const auto startTime = std::chrono::steady_clock::now();

    for (const auto& point : trajectory) {
        const auto commandTime = startTime + std::chrono::milliseconds(point.time_ms);
        const auto now = std::chrono::steady_clock::now();
        if (now < commandTime) {
            std::this_thread::sleep_until(commandTime);
        }

        for (size_t axis = 0; axis < AXIS_COUNT; ++axis) {
            const bool hasCommand =
                (point.positions[axis] != previous[axis]) || axisStreamingStarted[axis];
            if (!hasCommand) continue;

            const int velocity = std::max(1, point.velocities[axis]);
            if (!axisStreamingStarted[axis]) {
                const int startRet = FAS_MoveSingleAxisAbsPos(
                    nPortIDs_[axis],
                    iSlaveNos_[axis],
                    point.positions[axis],
                    velocity);
                if (startRet == FMM_OK) {
                    axisStreamingStarted[axis] = true;
                    std::this_thread::sleep_for(kCommandSpacing);
                    continue;
                }
                if (startRet != FMP_RUNFAIL) {
                    std::cout << axisName(axis) << " initial trajectory move failed: "
                              << startRet << std::endl;
                    return finishRun(false);
                }
                axisStreamingStarted[axis] = true;
            ...
            const int velRet = FAS_VelocityOverride(nPortIDs_[axis], iSlaveNos_[axis], velocity);
            const int posRet = FAS_PositionAbsOverride(nPortIDs_[axis], iSlaveNos_[axis],
                                                       point.positions[axis]);
            if (velRet != FMM_OK || posRet != FMM_OK) {
                const int restartRet = FAS_MoveSingleAxisAbsPos(
                    nPortIDs_[axis],
                    iSlaveNos_[axis],
                    point.positions[axis],
                    velocity);
                if (restartRet != FMM_OK && restartRet != FMP_RUNFAIL) {
                    std::cout << axisName(axis)
                              << " trajectory override failed. vel=" << velRet
                              << ", pos=" << posRet
                              << ", restart=" << restartRet << std::endl;
                    return finishRun(false);
                }
            }
            std::this_thread::sleep_for(kCommandSpacing);
