...
constexpr int kMaxTrackingErrorPulses = 2000;
...
bool trackingErrorExceeded(const AxisPositions& actual,
                           const AxisPositions& expected,
                           int maxErrorPulses,
                           size_t segmentIndex) {
    bool exceeded = false;
    for (size_t i = 0; i < AXIS_COUNT; ++i) {
        const auto error =
            std::llabs(static_cast<long long>(actual[i]) -
                       static_cast<long long>(expected[i]));
        if (error > maxErrorPulses) {
            std::cout << "Tracking error exceeded at segment " << segmentIndex
                      << ", axis " << (i + 1)
                      << ": actual=" << actual[i]
                      << ", expected=" << expected[i]
                      << ", error=" << error
                      << ", limit=" << maxErrorPulses << std::endl;
            exceeded = true;
        }
    }
    return exceeded;
...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    auto finishRun = [&](bool ok) {
        if (!ok) {
            stopAllAxes(nPortIDs_, iSlaveNos_);
        }
        if (timingSaved) {
            restoreAllMotionTiming(nPortIDs_, iSlaveNos_, kAccelerationTimeParameter, savedAccel, "accel time");
            restoreAllMotionTiming(nPortIDs_, iSlaveNos_, kDecelerationTimeParameter, savedDecel, "decel time");
    ...
    std::cout << "AD mode: planned-path override with actual tracking guard. "
              << "Max error: " << kMaxTrackingErrorPulses << " pulses."
              << std::endl;

    for (size_t i = 0; i < steps; ++i) {
        const auto segmentStart = std::chrono::steady_clock::now();
        auto nextCommandTime = segmentStart;
        AxisPositions actual{};
        AxisPositions plannedStart{};
        AxisPositions target{};
        AxisBools hasCommand{};

        if (!readActualPositions(actual)) {
            std::cout << "Failed to read actual positions before segment "
                      << i << "." << std::endl;
            return finishRun(false);
        }

        for (size_t a = 0; a < AXIS_COUNT; ++a) {
            plannedStart[a] = (i == 0) ? initialPos[a] : runPaths[a][i - 1];
            target[a] = runPaths[a][i];
            hasCommand[a] = (target[a] != plannedStart[a]);
        }

        if (trackingErrorExceeded(actual, plannedStart, kMaxTrackingErrorPulses, i)) {
            return finishRun(false);
        }

        AxisVelocities velocities =
            computeSegmentVelocities(plannedStart, target, segment_period_us);
