...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    constexpr auto command_spacing = std::chrono::microseconds(1500);
    constexpr auto segment_period = std::chrono::milliseconds(50);
    const auto command_window =
        std::chrono::duration_cast<std::chrono::microseconds>(command_spacing * AXIS_COUNT);
    const auto segment_period_us =
        std::chrono::duration_cast<std::chrono::microseconds>(segment_period);
    const auto segment_tail_delay =
        (segment_period_us > command_window) ? (segment_period_us - command_window)
    ...
    for (size_t i = 0; i < steps; ++i) {
        ...
        AxisVelocities velocities = computeVelocities(previous, target, hasCommand);
        ...
        previous = target;
        if (i + 1 < steps) {
            std::this_thread::sleep_for(segment_tail_delay);
        }
    }
