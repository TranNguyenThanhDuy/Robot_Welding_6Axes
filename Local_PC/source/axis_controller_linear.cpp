...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    for (size_t i = 0; i < steps; ++i) {
        ...
        unsigned char slaves[AXIS_COUNT];
        int absPos[AXIS_COUNT];
        for (size_t a = 0; a < AXIS_COUNT; ++a) {
            slaves[a] = iSlaveNos_[a];
            absPos[a] = target[a];
        }

        constexpr unsigned int linear_feedrate = 1000;
        constexpr unsigned short linear_accel_time_ms = 200;
        int ret = FAS_MoveLinearAbsPos2(
            nPortIDs_[0],
            static_cast<unsigned char>(AXIS_COUNT),
            slaves,
            absPos,
            linear_feedrate,
            linear_accel_time_ms);
        if (ret != FMM_OK) {
            std::cout << "FAS_MoveLinearAbsPos2 failed: " << ret << std::endl;
