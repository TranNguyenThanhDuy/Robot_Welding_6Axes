...
constexpr size_t kLookaheadWindow = 5;
constexpr double kMaxScaleDropPerSegment = 0.18;
constexpr double kMaxScaleRisePerSegment = 0.10;
...
double clampScaleStep(double previousScale, double desiredScale) {
    const double safePrevious =
        std::max(kMinCornerSpeedScale, std::min(1.0, previousScale));
    const double safeDesired =
        std::max(kMinCornerSpeedScale, std::min(1.0, desiredScale));

    if (safeDesired < safePrevious - kMaxScaleDropPerSegment) {
        return safePrevious - kMaxScaleDropPerSegment;
    }
    if (safeDesired > safePrevious + kMaxScaleRisePerSegment) {
        return safePrevious + kMaxScaleRisePerSegment;
    }
    return safeDesired;
}
...
double lookaheadSpeedScale(const AxisVectors& paths,
    ...
    const size_t lastCorner =
        (steps > 1) ? std::min(steps - 2, currentIndex + kLookaheadWindow - 1)
                    : currentIndex;
    ...
    for (size_t cornerIdx = currentIndex; cornerIdx <= lastCorner; ++cornerIdx) {
        ...
        scale = std::min(scale, cornerSpeedScale(before, corner, after));
    }
...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    double currentSpeedScale = 1.0;
    std::cout << "Lookahead VIP mode: window=" << kLookaheadWindow
              << ", min corner scale=" << kMinCornerSpeedScale
              << ", scale drop/segment=" << kMaxScaleDropPerSegment
              << ", rise/segment=" << kMaxScaleRisePerSegment << std::endl;
    ...
    for (size_t i = 0; i < steps; ++i) {
        ...
        const double desiredSpeedScale =
            (i + 1 < steps) ? lookaheadSpeedScale(runPaths, i, steps, previous) : 1.0;
        currentSpeedScale = clampScaleStep(currentSpeedScale, desiredSpeedScale);
        const auto active_segment_period_us =
            scaledSegmentPeriod(segment_period_us, currentSpeedScale);
        AxisVelocities velocities =
