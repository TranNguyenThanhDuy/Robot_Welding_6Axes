...
constexpr int kPositionTolerancePulses = 0;
constexpr int kMaxStreamingVelocity = static_cast<int>(base_velocity);
...
AxisVelocities computeSegmentVelocities(const AxisPositions& current,
    ...
    for (size_t i = 0; i < AXIS_COUNT; ++i) {
        ...
        if (speed > static_cast<double>(kMaxStreamingVelocity)) {
            velocities[i] = kMaxStreamingVelocity;
        } else {
...
bool AxisController::goWithBuffer(const AxisVectors& paths) {
    ...
    std::cout << "Override fix: actual feedback with planned-step velocity cap."
              << std::endl;
    ...
    for (size_t i = 0; i < steps; ++i) {
        ...
        for (size_t a = 0; a < AXIS_COUNT; ++a) {
            ...
            hasCommand[a] =
                std::llabs(static_cast<long long>(target[a]) -
                           static_cast<long long>(actual[a])) >
                kPositionTolerancePulses;
        }
