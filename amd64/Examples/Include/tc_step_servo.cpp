...
// Global variables
int nPortID1 = 0;  // Port for Motor 1 (EziSERVO)
int nPortID2 = 1;  // Port for Motor 2 (EziSTEP)
unsigned char iSlaveNo1 = 0;  // Motor 1 (EziSERVO)
unsigned char iSlaveNo2 = 1;  // Motor 2 (EziSTEP)
...
// Motor types for mixed servo/step configuration
enum MotorType { SERVO_MOTOR = 0, STEP_MOTOR = 1 };
MotorType motor1_type = SERVO_MOTOR;  // EziSERVO
MotorType motor2_type = STEP_MOTOR;   // EziSTEP
// MotorType motor3_type = SERVO_MOTOR, motor4_type = SERVO_MOTOR;  // For 6-axis
// MotorType motor5_type = SERVO_MOTOR, motor6_type = SERVO_MOTOR;  // For 6-axis

const unsigned int base_velocity = 1000;  // Base velocity for all movements
...
int main()
{
    ...
    while (true)
    {
        ...
        if (cmd == "on") {
            ...
            std::cout << "Unknown command. Use 'on', 'off', 'home', 'record', 'stop', 'clear', 'go', 'movepos', 'go pos', 'postable', 'print table', or 'q'." << std::endl;
        }
    }
...
void initializeSystem() {
    ...
    // Driver_Connection(sPort, dwBaudRate, nPortID2, iSlaveNo2);
...
void showCommands() {
    std::cout << "=== MIXED SERVO/STEP CONTROL ===" << std::endl;
    // std::cout << "=== 6-AXIS SERVO CONTROL ===" << std::endl;  // Uncomment for 6-axis
    std::cout << "Motor 1: EziSERVO, Motor 2: EziSTEP Plus-R" << std::endl;
    std::cout << "on: Enable motors (EziSERVO ON + EziSTEP ready)" << std::endl;
    // std::cout << "on: Enable all 6 motors" << std::endl;  // Uncomment for 6-axis
    std::cout << "off: Disable motors (EziSERVO OFF + EziSTEP STOP)" << std::endl;
    // std::cout << "off: Disable all 6 motors" << std::endl;  // Uncomment for 6-axis
...
bool handleServoOn() {
    // EziSERVO uses ServoOn, EziSTEP is always ready when powered (no servo enable needed)
    bool success1 = (motor1_type == SERVO_MOTOR) ? ServoOn(nPortID1, iSlaveNo1) : true;
    bool success2 = (motor2_type == SERVO_MOTOR) ? ServoOn(nPortID2, iSlaveNo2) : true;
    
    // bool success3 = (motor3_type == SERVO_MOTOR) ? ServoOn(nPortID3, iSlaveNo3) : true;  // For 6-axis
    // bool success4 = (motor4_type == SERVO_MOTOR) ? ServoOn(nPortID4, iSlaveNo4) : true;  // For 6-axis
    // bool success5 = (motor5_type == SERVO_MOTOR) ? ServoOn(nPortID5, iSlaveNo5) : true;  // For 6-axis
    // bool success6 = (motor6_type == SERVO_MOTOR) ? ServoOn(nPortID6, iSlaveNo6) : true;  // For 6-axis
    
    if (success1 && success2) {
    // if (success1 && success2 && success3 && success4 && success5 && success6) {  // For 6-axis
        printf("Motors enabled successfully - M1(EziSERVO): %s, M2(EziSTEP): %s\n",
               (motor1_type == SERVO_MOTOR) ? (success1 ? "ON" : "FAIL") : "Ready",
               (motor2_type == SERVO_MOTOR) ? (success2 ? "ON" : "FAIL") : "Ready");
        // printf("All 6 motors enabled successfully.\n");  // For 6-axis
        return true;
    } else {
        printf("Motor enable failed - M1(EziSERVO): %s, M2(EziSTEP): %s\n", 
               (motor1_type == SERVO_MOTOR) ? (success1 ? "OK" : "FAIL") : "Ready",
               (motor2_type == SERVO_MOTOR) ? (success2 ? "OK" : "FAIL") : "Ready");
        // printf("Motor enable failed - M1: %s, M2: %s, M3: %s, M4: %s, M5: %s, M6: %s\n",  // For 6-axis
...
bool handleServoOff() {
//     // EziSERVO: Use ServoOff()
//     // EziSTEP: Use FAS_StopAxis() to stop current movement
//     bool success1 = true, success2 = true;
    
//     if (motor1_type == SERVO_MOTOR) {
//         success1 = ServoOff(nPortID1, iSlaveNo1);
//     } else {
//         // For EziSTEP: Stop any motion 
//         FAS_StopAxis(nPortID1, iSlaveNo1);  // Stop current movement
//         success1 = true;  // Assume success for stepper stop
//     }
    
//     if (motor2_type == SERVO_MOTOR) {
//         success2 = ServoOff(nPortID2, iSlaveNo2);
//     } else {
//         // For EziSTEP: Stop any motion
//         FAS_StopAxis(nPortID2, iSlaveNo2);  // Stop current movement
//         success2 = true;  // Assume success for stepper stop
//     }
    
//     // bool success3 = (motor3_type == SERVO_MOTOR) ? ServoOff(nPortID3, iSlaveNo3) : (FAS_StopAxis(nPortID3, iSlaveNo3), true);  // For 6-axis
//     // bool success4 = (motor4_type == SERVO_MOTOR) ? ServoOff(nPortID4, iSlaveNo4) : (FAS_StopAxis(nPortID4, iSlaveNo4), true);  // For 6-axis
//     // bool success5 = (motor5_type == SERVO_MOTOR) ? ServoOff(nPortID5, iSlaveNo5) : (FAS_StopAxis(nPortID5, iSlaveNo5), true);  // For 6-axis
//     // bool success6 = (motor6_type == SERVO_MOTOR) ? ServoOff(nPortID6, iSlaveNo6) : (FAS_StopAxis(nPortID6, iSlaveNo6), true);  // For 6-axis
    
//     if (success1 && success2) {
//     // if (success1 && success2 && success3 && success4 && success5 && success6) {  // For 6-axis
//         printf("Motors disabled successfully - M1(EziSERVO): %s, M2(EziSTEP): %s\n",
//                (motor1_type == SERVO_MOTOR) ? (success1 ? "OFF" : "FAIL") : "STOPPED",
//                (motor2_type == SERVO_MOTOR) ? (success2 ? "OFF" : "FAIL") : "STOPPED");
//         // printf("All 6 motors disabled successfully.\n");  // For 6-axis
//         return true;
//     } else {
//         printf("Motor disable failed - M1(EziSERVO): %s, M2(EziSTEP): %s\n",
//                (motor1_type == SERVO_MOTOR) ? (success1 ? "OK" : "FAIL") : "STOPPED",
//                (motor2_type == SERVO_MOTOR) ? (success2 ? "OK" : "FAIL") : "STOPPED");
//         // printf("Motor disable failed - M1: %s, M2: %s, M3: %s, M4: %s, M5: %s, M6: %s\n",  // For 6-axis
//         //        success1 ? "OK" : "FAIL", success2 ? "OK" : "FAIL", success3 ? "OK" : "FAIL",
//         //        success4 ? "OK" : "FAIL", success5 ? "OK" : "FAIL", success6 ? "OK" : "FAIL");
//         return false;
//     }
